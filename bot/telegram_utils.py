...
def send_expiration_notification(user_id, db_manager):
    chat_id = get_chat_id_for_user(user_id, db_manager)
    if chat_id:
        message = "Ссылка для оплаты истекла. Пожалуйста, запросите новую."
        send_telegram_notification(user_id, message, db_manager)